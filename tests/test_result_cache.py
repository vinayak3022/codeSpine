...
def test_make_key_same_args_same_key():
    k1 = ResultCache.make_key("get_impact", {"symbol": "Foo", "project": None}, 1_000_000_000)
    k2 = ResultCache.make_key("get_impact", {"symbol": "Foo", "project": None}, 1_000_000_000)
    assert k1 == k2


def test_make_key_different_tool():
    k1 = ResultCache.make_key("get_impact", {"symbol": "Foo"}, 1_000_000_000)
    k2 = ResultCache.make_key("detect_dead_code", {"symbol": "Foo"}, 1_000_000_000)
    assert k1 != k2


def test_make_key_different_mtime():
    k1 = ResultCache.make_key("get_impact", {"symbol": "Foo"}, 1_000_000_000)
    k2 = ResultCache.make_key("get_impact", {"symbol": "Foo"}, 1_000_000_001)
    assert k1 != k2


def test_make_key_different_args():
    k1 = ResultCache.make_key("search", {"query": "Foo"}, 1_000_000_000)
    k2 = ResultCache.make_key("search", {"query": "Bar"}, 1_000_000_000)
    assert k1 != k2


def test_make_key_arg_order_independent():
    k1 = ResultCache.make_key("t", {"a": 1, "b": 2}, 0)
    k2 = ResultCache.make_key("t", {"b": 2, "a": 1}, 0)
    assert k1 == k2  # sorted keys in json.dumps


def test_make_key_preserves_rapid_snapshot_ns_changes():
    k1 = ResultCache.make_key("t", {}, 1_700_000_000_000_000_001)
    k2 = ResultCache.make_key("t", {}, 1_700_000_000_000_000_002)
    assert k1 != k2
...
def test_lru_eviction():
    ...
    for i in range(4):
        k = ResultCache.make_key("t", {"i": i}, 0)
        c.put(k, str(i))
    # First entry should have been evicted.
    k0 = ResultCache.make_key("t", {"i": 0}, 0)
    assert c.get(k0) is None
    # Later entries should still be present.
    k3 = ResultCache.make_key("t", {"i": 3}, 0)
    assert c.get(k3) == "3"


def test_lru_promotes_on_get():
    c = ResultCache(maxsize=3)
    keys = [ResultCache.make_key("t", {"i": i}, 0) for i in range(3)]
...
def test_invalidate_tool():
    c = ResultCache()
    k1 = ResultCache.make_key("get_impact", {"s": "A"}, 0)
    k2 = ResultCache.make_key("detect_dead_code", {}, 0)
    c.put(k1, "a")
    c.put(k2, "b")
